...
> **Note** — **Overdue takes the top of the scale, not the top of the type's own rows.** A type whose Settings rows are all `minor` still reports the **highest** band once it is past due; the ceiling comes from the severity scale, not from which windows an admin happened to write for that type. This **reverses** the earlier reading of "the highest **configured** severity" as a per-type restriction, which left a minor-only type at `minor` however far past due it went. The oracle is legacy: `reference/legacy/lib/email.ts` puts every item with `daysLeft < 0` into one bucket (:174), renders it as the red "🔴 Overdue — action needed now" section ahead of critical (:238–239), and counts all of it into the "⚠️ N urgent" subject line (:260) — for every item, regardless of type, because legacy has no per-type severity at all. Read as a per-type ceiling, a minor-only type could never reach the top band however far past due it ran, and under-warning is the silence this module exists to prevent. Legacy's own digest never collects an already-expired document, invoice or schedule — every collection query is `gte: now` — so the only items that reach its overdue bucket are unprocessed payroll runs. The oracle therefore settles the SEMANTICS (an overdue item is uniformly top-tier, ahead of critical, inside the urgent count) and not a volume comparison: on an expired visa legacy is silent, so this build was never quieter than it there. Ahmed's decision, 2026-08-16, reversing an earlier reading that a type's own rows capped its overdue severity.
>
