...
**"Files the gate measures" is everything git tracks or would track, minus two sets.** `package-lock.json`, which the `nolock` pathspec already excludes from both size budgets and which `coverage.include` never selects, so committing it moves no printed number. And anything `.gitignore` covers — `coverage/`, `node_modules/`, `.next/`, `generated/`, `.task-current.yaml` — which is the gate's own output and its deliberately uncommitted input, and can never appear in `$base...HEAD`. Everything else is measured, because `size-total` counts every added line of it: a dirty `docs/` page, a dirty `.claude/` prompt and a dirty backlog node each move a number the suite prints. The narrower rule considered and rejected was "refuse only for `lib/` and `tests/`", which would have passed the exact defect that prompted this record, `scripts/gate.sh` being neither. Untracked files count and are listed individually (`-uall`), since an untracked file in a measured path is precisely a file the committed diff lacks and the next commit will carry.
