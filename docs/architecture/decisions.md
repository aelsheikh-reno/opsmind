...
### ADR-035 · A comment is not implementation
...
**Decision.** **`size-impl` counts a `.ts` or `.tsx` line only when it carries code.** A comment-only line and a blank line are not implementation. **A line carrying code AND a trailing comment counts as code, in full** — no partial credit, because the alternative rewards moving an explanation onto the line it explains, which is the one place a reviewer is least likely to read it. `size-impl` stays at **400** and stays **never-waivable**. `size-total` is unchanged and still counts every added line of every file.

**The classification is the TypeScript compiler's, never a regex.** `//` inside a string literal, inside a template literal and inside a JSX expression is code, and only a parser can tell it from a comment. This repository has already been bitten by a line scanner reading declarations out of string literals, which is why `tests/kernel/kernel-source.ts` exists and parses with `ts.createSourceFile`; the classification is that reader's `classifyLines`, built on the comment ranges the parser reports. `scripts/size-impl.mjs` calls it rather than reimplementing it — **one implementation, used by both**, because two would drift and the suite would only ever grade one. There is no `tsx` or `ts-node` here, so the script transpiles the reader with `ts.transpileModule` and imports the JavaScript from `node_modules/.cache`, which is gitignored and sits exactly two levels below the repository root so that the copy resolves both its `typescript` import and the reader's own `REPO_ROOT` — asserted at load rather than assumed. A `.tsx` file is parsed as TSX: under the TS kind `<div>{x}</div>` is a type assertion, JSX text is not JSX text, and the comment ranges come back describing a file nobody wrote.
...
**Consequences.** Explaining a change costs nothing, and correcting a comment that has become false costs nothing, which is what the annotation finding says the gate should want. The budget now measures the thing the 400 was calibrated on: code a reviewer has to hold in their head.

**The cost is real and it is charged to `size-total`.** A comment-heavy diff is still a large diff to read, and 450 lines of which 100 are prose still takes a reviewer longer than 350. `size-impl` no longer sees that; `size-total` does, unchanged at 1500 and counting every line, and it is now the only budget holding that line. The narrower risk accepted is that a task can grow its explanation without limit until the backstop notices, and that a *wrong* comment is now free — the gate has never been able to judge whether an annotation is true, and it can no longer even charge for its volume.

**Two operational costs.** `size-impl` is no longer a `git diff --numstat` one-liner: it needs node, the `typescript` package and the reader, so there are three more ways for the measurement to fail for infrastructure reasons — deliberately, since every one of them FAILs rather than passes. And the fixtures that probe the size gate now need what a real run needs. `scripts/test-guards.sh` builds its budget probes as a copy of `scripts/` in a temp directory with no `node_modules` and no `tests/`, so the six probes whose fixture carries a `.ts` file cannot classify and refuse. The fix is the one [ADR-031](#adr-031) already recorded for the same file — make the fixture the state a real run requires, by symlinking `node_modules` and copying the reader in, rather than softening the script for an environment missing its toolchain. `tests/gates/size-impl-comments.test.ts` builds its gate fixtures exactly that way and demonstrates that it works. **That change to `scripts/test-guards.sh` is outside this task's `produces` list and has not been made here**, so the `guards` line is red until it is. It is recorded as `gate-size-impl-guard-fixtures`, with the six affected probes named on the node; if that node is gone without its fix having landed, the guard suite is measuring less than it claims to. It is the first thing to fix, and it is not a defect in the measurement.
