...
// was merged with `module-deadlines`; the sweep half (the store and alert ports,
// the `World`, the sweep verbs) was carried out of that PR as `surface.full.ts`
// and is folded back in here under its final name. Where the two disagreed the
// MERGED half won, because it was the later and better-reasoned of the two:
// `ymd` reads UTC parts rather than local ones, which is what stopped ten
// calendar tests from depending on the runner's timezone.
//
...
// declarations of the merged `index.ts` — names, parameter lists and types, no
// bodies. That is the module's public surface, which the task explicitly allows.
//
...
//
// TOLERANT PLUMBING, STRICT ASSERTIONS. Three things this node settles have no
// spelling fixed anywhere in the spec: the timezone field on BusinessCalendar,
// the scope declaration a run attaches to its report, and the extra arguments
// `raiseAlert` takes. The plumbing below accepts more than one spelling of each
// so a naming disagreement between this file and the module surfaces as a
// failing assertion that names the rule, rather than as a compile error that
// takes the whole suite — including the merged calendar tests — down with it.
// Nothing about WHAT is asserted is relaxed by that: the information the spec
// requires is still required, and every finder proves itself non-empty first.
import {
  businessDaysUntil,
 ...
 * build stores civil dates (data-model.md:19) and how Prisma `@db.Date` returns
 ...
 * (data-model.md:19), so the day a Date names is its UTC day. Reading LOCAL
 * parts off a UTC-midnight Date is correct only at offsets at or east of UTC —
 * it silently reports the previous day anywhere west, which made ten of these
...
/**
 * A BusinessCalendar with the civil timezone this node adds to it.
 *
 * tasks/backlog.yaml#module-deadlines-sweep — "the jurisdiction's civil date
 * defines today, and the timezone lives on BusinessCalendar beside the weekend
 * mask and holidays — Jurisdiction is identity, BusinessCalendar is the
 * civil-time rules." Declared as an intersection so this file compiles whether
 * or not the module has landed the field yet; a module that ignores it fails the
 * civil-date tests loudly instead of failing to build.
 */
export type TestCalendar = BusinessCalendar & { timeZone?: string };

/**
 * `timeZone` defaults to UTC so that every test which is not ABOUT the timezone
 * keeps its expected values: a UTC-midnight `today` names the same civil day in
 * UTC as it does at any offset east of it. The tests that are about it pass
 * "Asia/Dubai" explicitly, which is where the day-late defect lives.
 */
...
export function evaluationOf(
  ...
  if (result?.severity != null) {
    return { fingerprint: result.fingerprint, severity: result.severity };
  }
