...
import { askAboutDocument, type DocumentExtraction } from "@/lib/extract";
...
export async function POST(request: NextRequest) {
  ...
  const { tempFilePath, mimeType, filename, extraction, question, chatHistory } = body;

  if (!tempFilePath || !mimeType || !filename || !extraction || !question) {
  ...
  let answer: string;
  try {
    answer = await askAboutDocument(buffer, mimeType, filename, extraction, question, chatHistory ?? []);
  } catch (err) {
    ...
    return NextResponse.json({ error: `Question failed: ${message}` }, { status: 500 });
  }
